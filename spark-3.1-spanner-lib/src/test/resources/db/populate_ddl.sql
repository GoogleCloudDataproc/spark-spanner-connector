...
CREATE INDEX ATableByB ON ATable (A, B) STORING (C, D, E, F, G, H, I, J, K, L);
...
CREATE INDEX OrdersByCustkey ON ORDERS (O_ORDERKEY, O_CUSTKEY) STORING (O_ORDERSTATUS, O_TOTALPRICE, O_ORDERDATE, O_ORDERPRIORITY, O_CLERK, O_SHIPPRIORITY, O_COMMENT);
...
CREATE INDEX LineitemByReturnFlag ON LINEITEM (O_ORDERKEY, L_LINENUMBER, L_RETURNFLAG) STORING (L_PARTKEY, L_SUPPKEY, L_QUANTITY, L_EXTENDEDPRICE, L_DISCOUNT,
    L_TAX, L_LINESTATUS, L_SHIPDATE, L_COMMITDATE, L_RECEIPTDATE, L_SHIPINSTRUCT, L_SHIPMODE, L_COMMENT);
CREATE INDEX LineitemJoinTestIndex ON LINEITEM(O_ORDERKEY) STORING (L_PARTKEY, L_SUPPKEY, L_QUANTITY, L_EXTENDEDPRICE, L_DISCOUNT,
...
CREATE TABLE UUID_CHILD (
  ...
  INTERLEAVE IN PARENT UUID_PARENT ON DELETE CASCADE;
